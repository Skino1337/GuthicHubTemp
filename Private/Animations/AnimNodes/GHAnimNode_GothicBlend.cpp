...
void FAnimNode_GothicBlend::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
...
void FAnimNode_GothicBlend::RebuildPerBoneBlendWeights(const USkeleton* InSkeleton, const UObject* AnimInstanceObject)
{
	if (InSkeleton)
	{
		FInputBlendPose InputBlendPose = FInputBlendPose();
		
		auto AnimInstance = Cast<UGHAnimInstance>(AnimInstanceObject);
		if (AnimInstance)
		{
			auto CurrentAnimationData = AnimInstance->CurrentAnimationDataLayerMap.FindRef(LayerName);
			if (CurrentAnimationData)
			{
				if (CurrentAnimationData->BlendBones.Num() > 0)
				{
					for (auto BlendBone : CurrentAnimationData->BlendBones)
					{
						FBranchFilter BranchFilter = FBranchFilter();
						BranchFilter.BoneName = FName(BlendBone);
						BranchFilter.BlendDepth = 0;
						
						InputBlendPose.BranchFilters.Add(BranchFilter);
					}
				}
			}
		}
		
		TArray<FInputBlendPose> BlendFilterArray = {InputBlendPose};

		FAnimationRuntime::CreateMaskWeights(PerBoneBlendWeights, BlendFilterArray, InSkeleton);
...
bool FAnimNode_GothicBlend::ArePerBoneBlendWeightsValid(const USkeleton* InSkeleton) const
{
	return (InSkeleton != nullptr && InSkeleton->GetGuid() == SkeletonGuid && InSkeleton->GetVirtualBoneGuid() == VirtualBoneGuid);
}

void FAnimNode_GothicBlend::UpdateCachedBoneData(const FBoneContainer& RequiredBones, const USkeleton* Skeleton, const UObject* AnimInstanceObject)
{
	auto AnimInstance = Cast<UGHAnimInstance>(AnimInstanceObject);
	if (AnimInstance)
	{
		...
		if (CurrentAnimationData)
		{
			if (CurrentBoneNameArray.Num() != CurrentAnimationData->BlendBones.Num())
			{
				RequiredBonesSerialNumber = 0;
				CurrentBoneNameArray = CurrentAnimationData->BlendBones;
				RebuildPerBoneBlendWeights(Skeleton, AnimInstanceObject);
			}
			else
			{
				for (int32 i = 0; i < CurrentBoneNameArray.Num(); i++)
				{
					auto BoneName = CurrentBoneNameArray[i];
					if (!CurrentAnimationData->BlendBones.Contains(BoneName))
					{
						RequiredBonesSerialNumber = 0;
						CurrentBoneNameArray = CurrentAnimationData->BlendBones;
						RebuildPerBoneBlendWeights(Skeleton, AnimInstanceObject);
	...
	if (TotalCount > 0)
	{
		...
		CurvePoseSourceIndices.Init(0xFF, TotalCount);
...
void FAnimNode_GothicBlend::CacheBones_AnyThread(const FAnimationCacheBonesContext& Context)
{
	...
	UpdateCachedBoneData(Context.AnimInstanceProxy->GetRequiredBones(), Context.AnimInstanceProxy->GetSkeleton(), Context.GetAnimInstanceObject());
}

void FAnimNode_GothicBlend::Update_AnyThread(const FAnimationUpdateContext& Context)
{
...
void FAnimNode_GothicBlend::Evaluate_AnyThread(FPoseContext& Output)
{
...
void FAnimNode_GothicBlend::GatherDebugData(FNodeDebugData& DebugData)
{
...
void FAnimNode_GothicBlend::SetBlendMask(int32 InPoseIndex, UBlendProfile* InBlendMask)
{
